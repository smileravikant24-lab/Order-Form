...
// ✅ UPDATED doPost: Deduplication via submissionId cache + LockService
function doPost(e) {
  var lock = LockService.getScriptLock();
  try {
    // 1. Wait up to 30 seconds for any concurrent request to complete
    lock.waitLock(30000);

    var data = JSON.parse(e.postData.contents);

    // 2. DUPLICATE CHECK USING SUBMISSION ID:
    // If browser retries or user double clicks, the exact same submissionId is checked
    if (data.submissionId) {
      var cache = CacheService.getScriptCache();
      var cacheKey = 'SUB_ID_' + data.submissionId;
      var cachedResult = cache.get(cacheKey);
      if (cachedResult) {
        Logger.log('DUPLICATE REQUEST BLOCKED BY SUBMISSION_ID: ' + data.submissionId);
    ...
    if (data.formType === 'Order' || data.formType === 'Enquiry' && data.items) {
      ...
      result = processEnquiry(formData, new Date());
    }

    var responseString = JSON.stringify(result);

    // 3. Cache the successful response for 10 minutes (600 seconds)
    if (data.submissionId && result && result.success) {
      try {
        CacheService.getScriptCache().put('SUB_ID_' + data.submissionId, responseString, 600);
      } catch (cErr) {
        Logger.log('Cache put error: ' + cErr.message);
      }
    }
